export default async function handler(req, res) {
    ...
    const { nome, cpf, telefone, valor, diaVencimento, codigo } = req.body;
    ...
    try {
      ...
      const buscaResp = await fetch(`${ASAAS_URL}/customers?cpfCnpj=${cpfLimpo}`, { headers });
      ...
      if (busca.data && busca.data.length > 0) {
        ...
        if (!clienteResp.ok) {
          return res.status(400).json({ error: cliente.errors || "Erro ao criar cliente" });
        }
        clienteId = cliente.id;
      }
  
      const hoje = new Date();
      const dia = Math.min(diaVencimento || 5, new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0).getDate());
      ...
      if (!cobrancaResp.ok) {
        return res.status(400).json({ error: cobranca.errors || "Erro ao criar cobrança" });
      }
  
      return res.status(200).json({ link: cobranca.invoiceUrl, vencimento: vencimentoStr });
    } catch (err) {
